...
;-------------------------------------------------------------------------------
; DllSleep - High precision sleep using Winmm.dll from PTCGPB Community Developer Crinity 2026.1.31
;-------------------------------------------------------------------------------
DllSleep(ms) {
...
;===============================================================================
; MuMu Instance Management Functions
;===============================================================================

;-------------------------------------------------------------------------------
; LaunchMuMuInstance - Launch MuMu instance using proper method
;-------------------------------------------------------------------------------
LaunchMuMuInstance(instanceName, folderPath) {
    ; Determine MuMu folder
    mumuFolder := folderPath . "\MuMuPlayerGlobal-12.0"
    if !FileExist(mumuFolder)
        mumuFolder := folderPath "\MuMu Player 12"
    if !FileExist(mumuFolder)
    ...
    ; Get actual MuMu instance number from player name
    mumuNum := getMumuInstanceNum(instanceName, mumuFolder)
    if (mumuNum != "") {
